...
class Minimax(object):
    ...
    def bestMove(self, depth, state, curr_player):
        ...
        for col in range(7):
            ...
            if self.isLegalMove(col, state):
                ...
                legal_moves[col] = -self.search(depth-1, temp, opp_player,alpha,beta)
    ...
    def search(self, depth, state, curr_player,alpha,beta):
        ...
        if opp_player == self.colors[1]:
            for child in legal_moves:
                if child == None:
                   print("child == None (search)")
                beta = min(beta, self.search(depth-1, child, curr_player,alpha,beta))
                if alpha >= beta:
                    return self.value(state, curr_player)
            return beta
        else:
            for child in legal_moves:
                if child == None:
                    print("child == None (search)")
                alpha = max(alpha, -self.search(depth-1, child, opp_player,alpha,beta))
                if alpha >= beta:
                    return self.value(state, curr_player)
            return alpha
    ...
    def value(self, state, color):
        ...
        if opp_fours > 0:
            ...
            return my_fours*100000 + my_threes*100 + my_twos - opp_threes*100 - opp_twos
